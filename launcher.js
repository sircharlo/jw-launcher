  ...
  if (prefs.broadcastLang) {
    ...
    try {
      ...
      for (var featuredVideo of allVideos) {
        videos++;
        var featuredVideoElement = $("<div class='mt-0 pt-2'><div class='flex-column flex-fill h-100 rounded' data-url='" + featuredVideo.files.slice(-1)[0].progressiveDownloadURL + "' style='display: flex; background-image: url(\"" + featuredVideo.images.pnr.lg + "\"); background-size: cover; background-position: center;'><div class='flex-column flex-fill p-2' style='display: flex; background: linear-gradient(to right, rgba(0,0,0,0.9), rgba(0,0,0,0.6));'><div><h5 class='kbd'><kbd style='background-color: white; color: black;'>" + String.fromCharCode(65 + videos) + "</kbd></h5></div><div class='align-items-center flex-fill flex-row' style='display: flex;'><h5 style='color: white; white-space: normal; word-wrap: break-word;'>" + featuredVideo.title + "</h5></div></div></div></div>").click(function () {
